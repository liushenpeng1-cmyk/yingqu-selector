...
**yingqu 线上**: https://yqfmj.top(主域) 或 https://yingqu-selector.vercel.app(备用,相同 build)
...
- 落地时校验 `HTTP Referer` 包含以下任一合法域名,否则标 `unverified_referer`:
  - `yqfmj.top`(主域)
  - `yingqu-selector.vercel.app`(备用 Vercel 域名,serve 字节级相同的 build)

  示例代码:
  ```ts
  const YINGQU_DOMAINS = ["yqfmj.top", "yingqu-selector.vercel.app"];
  const refOK = YINGQU_DOMAINS.some(d => (document.referrer || "").includes(d));
  ```
...
```
GET https://yqfmj.top/api/catalog/schools
# 等价:GET https://yingqu-selector.vercel.app/api/catalog/schools
```
...
```
GET https://yqfmj.top/api/catalog/programs           # 全部
GET https://yqfmj.top/api/catalog/programs?target=pg  # 仅硕士
GET https://yqfmj.top/api/catalog/programs?target=ug  # 仅本科
# yingqu-selector.vercel.app 同样可用,返回内容字节级相同
```
...
- 当前 **v1.5**(JSON schema + catalog API + 双域名支持 + AP 课程体系 + langNoScore)
- 破坏性变更 → `v: 2` + 文档升版
...
- [ ] 拉一次 `https://yqfmj.top/api/catalog/schools` 存进你们 DB(§7.5)
- [ ] 拉一次 `https://yqfmj.top/api/catalog/programs?target=pg` 和 `?target=ug`(§7.5)
